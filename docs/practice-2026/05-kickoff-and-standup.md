# 启动会与每日站会模板

## 1. 60 分钟启动会议程

| 时间 | 议题 | 必须输出 |
| --- | --- | --- |
| 0-10 分 | 对齐评分与原则 | 前 5 天 40 分优先；不扩大 UC；不先拆服务 |
| 10-20 分 | 审阅 UC01-UC06 | 修改意见、教师确认问题、需排除功能 |
| 20-30 分 | 实名映射 A-E | 主责、协作者、Reviewer、备份人、可用时间 |
| 30-40 分 | 冻结平台选择 | CI、Docker 主机、K8s、镜像仓库、Secrets 方案 |
| 40-50 分 | 导入任务包 | 19 张首轮任务卡、截止、DoD、证据 |
| 50-60 分 | 确认 D1 Gate | 全 UC smoke、教师确认、基线标签、测试/容器骨架 |

## 2. 实名分工表

| 角色 | 姓名 | 主责 | 协作/Review | 8/25-9/4 可用时间 | 备份人 | 已确认 |
| --- | --- | --- | --- | --- | --- | --- |
| 组长 / A 平台与集成 | 林明（23375181） | ARCH-01、MS-00、Docker、CI、K8s、网关、HPA、性能、节奏 | E/王一涵 review 平台；A review 跨服务 | 待补 | 待补 | [x] 用户授权默认映射 |
| B 身份与社区 | 刘钟屹（23375291） | MS-01 identity-community、UC01、UC04 用户侧 | C/李晓萌 review | 待补 | 待补 | [x] 用户授权默认映射 |
| C 内容与媒体 | 李晓萌（24371422） | MS-02 content-media、UC02/03/04 | B/刘钟屹 review | 待补 | 待补 | [x] 用户授权默认映射 |
| D 直播与礼物 | 张壮志（24371350） | MS-03 live-reward、UC05、SRS/录播/账本 | A/林明 review | 待补 | 待补 | [x] 用户授权默认映射 |
| E 治理、质量与文档 | 王一涵（24371063） | MS-04 governance-ai、UC06、contract tests、REG-01、追溯/交付 | D/张壮志 review；全员协作回归 | 待补 | 待补 | [x] 用户授权默认映射 |

## 3. 启动会决策记录

| 决策项 | 结论 | 负责人 | 截止 | 证据 |
| --- | --- | --- | --- | --- |
| 最终 UC 范围 | UC01–UC06 已确认并在 `monolith-start` 基线全部 PASS | 组长 + E | 已完成 | `01-use-case-scope.md`、`03-smoke-checklist.md` |
| CI 平台 | Jenkins + Kind 为自动流水线；GitHub Actions 保留手动入口 | A | 已完成 | Jenkins Build #2/#4/#5/#7、`11-jenkins-kind-cicd-runbook.md` |
| K8s 环境 | 本地 Colima/Docker + Kind；迁移、部署、探针和回滚脚本已验证 | A | 已完成 | `07-environment-decision.md`、`11-jenkins-kind-cicd-runbook.md` |
| 镜像版本 | 使用 Git SHA，不只使用 `latest` | A | 已完成 | Jenkins/Kind 证据 |
| 微服务边界 | 四服务、31 表唯一 owner、接口、失败策略和迁移顺序已冻结 | 全员 | 2026-08-27 已完成 | `08-service-boundaries-and-data-ownership.md` |
| 内部鉴权 | 服务账号 JWT + K8s Secret | A + 各 owner | 第二阶段执行 | ARCH-01 第 0 节 |
| 数据迁移 | 停写窗口 + 可重复执行脚本；不做复杂双写 | A + 各 owner | 第二阶段执行 | ARCH-01 第 0/7 节 |
| 直播消息留存 | 7 天、每 Session 最多 10,000 条；业务事实不随聊天清理 | D | MS-03 执行 | ARCH-01 第 0 节 |
| PR/Review 规则 | A→E、B→C、C→B、D→A、E→D；Gateway 由组长和两侧 owner 共同 review | 组长 | 已冻结 | `04-task-board.md` 第二阶段分工 |

### 3.1 ARCH-01 评审记录

- 日期：2026-08-27。
- 结论来源：组长在当前项目协作会话中确认评审会已完成，全员同意默认方案。
- 角色确认：组长随后明确确认本人承担 A（平台与集成）。2026-08-31 用户提供五名成员及学号表，并授权按表格从上到下默认对齐 A–E：林明、刘钟屹、李晓萌、张壮志、王一涵。
- 技术结论：四服务边界、31 表唯一 owner、七项默认决策、迁移/回滚顺序和交叉 Review 关系冻结。
- 执行分工：A 负责平台/MS-00，B/C/D/E 分别负责 MS-01/MS-02/MS-03/MS-04，E 兼任 REG-01 与证据协调。
- 证据限制：仓库已取得 A–E 默认姓名/学号映射，但该表不能证明实际参会者、个人签字、签到、会议录屏或聊天截图；这些外部材料仍由组长后续补入证据索引。
- 变更控制：后续修改 owner、表归属、内部 contract 或迁移顺序必须新增决策记录并由受影响 owner 重新评审。
- 执行清单：各角色拿到分支后的任务、禁止事项、Reviewer 和统一 DoD 见 `12-second-stage-todo.md`。

## 4. 每日 5 分钟站会

每个人只回答三件事：

1. 昨天完成了什么？附任务 ID 和证据。
2. 今天唯一主目标是什么？写到可在当天验证。
3. 现在卡在哪里？谁在什么时候解除阻塞？

### 个人记录模板

```text
姓名：
日期：
昨天：完成 [任务 ID]，结果/证据：
今天：完成 [任务 ID] 的 [可验证目标]：
阻塞：
需要谁在何时协助：
风险变化：
```

### 组长约 200 字简报模板

```text
【第 X 组站会简报｜2026-XX-XX】
昨日：完成……，对应任务……，已有证据……。
今日：A 负责……；B 负责……；C 负责……；D 负责……；E 负责……。
当前阻塞：……，由……在……前处理。
风险：……。
今日 Gate：……；若未达到，止损方案为……。
看板/证据链接：……。
```

## 5. 会议结束检查

- [x] 每个第二阶段主任务只有一个角色 owner。
- [x] 每个负责人已有默认 Reviewer；真实备份人仍待组长补录。
- [ ] 每个人的 D1 目标能在当天验证。
- [x] Docker/K8s 已在当前 Mac 的 Colima + Kind 环境完成验证；第二阶段继续由 A 负责。
- [ ] 教师确认消息有发送人和截止时间。
- [ ] 次日 9:00 站会和 12:00 简报的实名责任人待组长补录。
