...
| 2026-08-29 | MS-04 PR #47 审查修复 | 修复 Unicode 身份头、KEEP 非 no-op、文本目标 ID、视频快照/Dashboard 缺字段和补偿并发租约；新增数据库并发测试与真实前端 services-mode smoke | governance/gateway/content/live 聚焦测试 62/62；受影响服务及前端 lint；governance/gateway/content/frontend build；Playwright 用例发现；shell/diff check | LOCAL PASS；隔离 MySQL integration 与 Compose/真实浏览器 smoke NOT RUN；保持 Draft 等待复审 |
