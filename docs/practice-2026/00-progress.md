...
- [ ] `CI-02-JUNIT` 生成并由 Jenkins 发布标准 JUnit XML。
  ...
  - 未打勾原因：尚未 push 到远端并形成真实 Jenkins Test Result 页面；Build 9401 的后续 Docker 镜像 `npm ci` 因网络持续无输出被主动终止，临时 MySQL 已清理，Kind 尚未创建，不能冒充完整流水线成功。
...
| CI-02-JUNIT 标准测试报告 | PARTIAL | requirements/Jest/Vitest/API/Playwright 生成 11 份 JUnit XML；Jenkins `junit` 发布配置；reporter 单测 | Build 9402：11 XML/186 tests；9305：exit 42 + 9 XML/167 tests；Jenkinsfile 在线校验 | LOCAL PASS；REAL JENKINS BUILD PENDING；完整镜像阶段网络 BLOCKED | 本轮待提交 |
...
| 2026-08-28 | CI-02-JUNIT 本地报告验证 | 为 requirements、Jest、Vitest、六微服务、API 和 Playwright 生成标准 JUnit XML；Jenkins `post` 配置 `junit` 发布；补 reporter 转义/失败/跳过单测 | Build 9402：11 XML/186 tests；Build 9305：故意失败 exit 42 + 9 XML/167 tests；`xmllint`；Jenkins 在线语法与插件检查 | PARTIAL PASS；全部 testcase 位于 testsuite，0 failures/errors；Docker 镜像内 `npm ci` 网络无进展后终止并清理；真实 Jenkins Test Result 页面待 push 后验证 |
