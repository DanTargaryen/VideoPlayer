...
- [x] `MS-04` 提取剩余 governance-ai 微服务并独立构建、测试和部署。
  ...
  - 当前结论：E 部分代码已完成；本轮 74 项聚焦测试、三处 lint/build 和 Prisma schema 校验 PASS。更新后的 migration、Compose/Playwright services-mode smoke 本轮未运行；PR 保持 Draft，等待非作者复审和远端 Check。治理写流量仍可通过 `GATEWAY_ROUTE_MODE=monolith` 回滚；未删除单体表，也未调用外部付费 AI。
...
| MS-04 governance-ai + REG-01 UC06 | DONE | 独立治理库、可信 Gateway 身份、创作者幂等提审、驳回原因/发布时间回写、隔离的待审队列、公开举报/审核 API、identity 通知、租约补偿和真实前端 services-mode 回归入口 | content/governance/frontend lint/build；聚焦测试 74/74；Prisma schema validate；完整 PR diff-check；更新后的 migration/Compose/Playwright NOT RUN | LOCAL PASS；保持 Draft，等待非作者复审和远端 Check | 本轮修复提交 |
