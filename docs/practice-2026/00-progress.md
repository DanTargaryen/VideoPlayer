...
- [ ] `MS-01..04` 提取四个业务微服务并独立构建、测试和部署。
  ...
  - 执行清单：`docs/practice-2026/12-second-stage-todo.md`；MS-00 已完成，C 已在 `feature/MS-02-content-media` 完成 content-media foundation 的 schema、只读 API、内部 contract、identity mock/fallback、真实 `ffprobe` 媒体边界实现和独立 Docker image/health/version 验收；真实 content 测试库 migration、真实 MinIO object 和 Gateway 切流仍为 `NOT RUN`。
...
| MS-02 content-media foundation | IN PROGRESS | 独立 content Prisma schema/migration/fixture；推荐、搜索、详情、相关推荐只读 API；review-decision/text-status/replay/batch-summary 内部 API；identity batch-summary mock + timeout fallback；媒体扩展名/MIME 与真实 `ffprobe` 视频流验证、写库失败清理 helper；独立容器 smoke 脚本 | `npm --workspace @videoplayer/content-media run lint`; `npm --workspace @videoplayer/content-media run build`; `npm --workspace @videoplayer/content-media run test`; `npm run test:services:ci`; Prisma validate with local placeholder `CONTENT_DATABASE_URL`; Docker image + `live`/`ready`/`version` + real MP4 `ffprobe` smoke | PASS；content Vitest 12/12；此前 services matrix PASS；`video-player/content-media:verify` 实际构建，`ffprobe 5.1.9`、真实 MP4 探测及 `live`/`ready`/`container-smoke` version PASS。真实数据库 migration、真实 MinIO、Gateway 切流仍 `NOT RUN` | 待提交 |
...
| 2026-08-27 | MS-02 content-media foundation | 在 `feature/MS-02-content-media` 建立 C 负责服务的独立 schema、fixture、只读公开接口、内部 contract、identity mock/fallback 和媒体边界 helper；将签名判断替换为真实 `ffprobe`，运行时镜像安装 `ffmpeg`，并加入独立容器 smoke；不切上传/投稿/互动写流量，不迁移 VideoAi 写路径，不删除单体表 | `npm --workspace @videoplayer/content-media run lint`；`npm --workspace @videoplayer/content-media run build`；`npm --workspace @videoplayer/content-media run test`；实际 Docker build；临时容器 `live`/`ready`/`version`；镜像内 `ffprobe -version`；真实 MP4 服务探测；Prisma validate with local placeholder `CONTENT_DATABASE_URL` | PASS；lint 0 errors、build PASS、content Vitest 1 文件 12/12；Docker Desktop 重启后基础镜像拉取 PASS，`video-player/content-media:verify` 构建 PASS，`ffprobe 5.1.9`、真实 MP4 探测和三个端点 PASS；真实 content 测试库 migration、真实 MinIO 和 Gateway 路由切换仍 `NOT RUN` |
