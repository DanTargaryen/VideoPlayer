...
| 2026-09-01 | QA-CLOSE-01 全接口/浏览器/Actions 闭环 | 自动发现并逐条请求 126 个公开 Controller 路由；新增 services-mode UC01–UC06 串行浏览器回归且 Token 缺失直接失败；Actions 在 PR/main 均构建单体、四服务、Gateway 与 migration Git SHA 镜像并部署隔离 Kind；修复“全部”分类误传、关注状态跨服务丢失与 Runner 磁盘峰值 | `npm run test:ci` 284/284；隔离 MySQL `npm run test:api` 143/143（Controller 126/126、0 skip、0 5xx）；标准 Compose Playwright 8/8、0 skip；本地 Kind 四 migration、单体 + 五服务/Gateway 全 Ready、0 restart、health/version PASS | LOCAL PASS；远端 PR Actions 与 Artifact 尚待本分支 push 后验证，未提前标记 REMOTE PASS |
