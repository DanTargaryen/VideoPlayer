...
| 2026-08-29 | CI-02 Jenkins WSL 本地配置 | 增加 WSL2 本地 Jenkins WAR 启停脚本与 Node 22 工具链说明；Jenkins 从 SCM 检测当前任务分支 `test/20260829`；修复 live-reward 内存消息清理的平方级性能问题 | `PATH="$PWD/.ci-local/node-v22.22.2-linux-x64/bin:$PATH" CI=true npm run test:ci`；Jenkins `127.0.0.1:8081`；Docker/Compose/Kind/kubectl 检查；Shell 语法检查 | PARTIAL；本地 CI 207/207 全部通过，Jenkins WAR 已启动；Jenkins 完整 DB/API/E2E/镜像/Kind 首次任务尚待 UI 配置后运行 |
| 2026-08-29 | CI-02 Jenkins Build #5 健康测试超时修复 | Jenkins 的 `ci-bootstrap.sh` 为单元测试注入 `LIVE_REWARD_DATABASE_URL`，导致“未配置持久化”用例误连尚未启动的 MySQL；让该测试临时隔离并恢复环境变量，保持失败阻断逻辑不变 | Jenkins Build #5 日志（live-reward 1/2 failed、8044ms、后续阶段 skipped）；带 `LIVE_REWARD_DATABASE_URL` 和 JUnit reporter 的 live-reward test | PARTIAL；修复后本地同构测试 18/18 通过；需在 Jenkins 重新构建确认完整流水线 |
