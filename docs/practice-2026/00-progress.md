...
- [x] `CI-AUTO-01` 恢复 PR 与主干 push 自动触发，并限制 PR 的部署范围。
  ...
  - 结果：PASS。手动完整 run `33462577980` 3/3 SUCCESS 并实检两份 Artifact；PR #64 自动 `pull_request` run `33463103266` 的 quality/public E2E SUCCESS、Kind 按设计 SKIPPED；合并为 `main@7fa3ed7` 后自动 `push` run `33463319512` 3/3 SUCCESS。最终 main Artifact 使用完整 merge SHA 镜像，2 migrations、backend/frontend/MySQL 1/1、0 restart、PVC Bound、cleanup SUCCESS。
...
- [x] `REG-01` 完成单体与微服务 Gateway 的 UC01–UC06 双目标全量回归。
  - 完成内容：将 `reg-01/v1` 的 UC06-only runner 升级为 v2；完整模式为每个目标创建独立 creator/actor/media 数据，按依赖顺序执行账户资料、投稿审核发布、发现观看、互动通知、直播录播和举报处置，最终按 UC01–UC06 固定顺序输出 PASS/FAIL/BLOCKED/NOT RUN。报告新增每个 UC 的公开 endpoint 覆盖清单；任何业务 FAIL 非零退出，`REG_REQUIRE_ALL_PASS=true` 同时拒绝已配置目标的 BLOCKED/NOT RUN。
  - 测试/验证：完整 `npm run test:ci` 278/278（REG harness 4/4）。标准 Compose 四库/MinIO/五服务/Gateway + 独立临时单体 MySQL/migration/seed/backend 3200；同一 runner 对 `MONOLITH_BASE_URL` 和 `MICROSERVICE_GATEWAY_BASE_URL` 各跑 6 项，最终 12/12 PASS。微服务先独立 6/6 PASS；双目标首轮单体 UC01–UC05、微服务 UC01–UC06 PASS，修正重复处置 400/409 兼容后双目标完整复跑 PASS；最终 monolith rollback 与所有资源清理 PASS。
  - 失败与修复：content 搜索冻结 contract 使用 `video` 而非 `videos`，runner 增加响应列表归一化；单体已处理举报返回 400、governance 返回 409，两者均为基线定义的明确拒绝，兼容断言后继续检查 target 保持 PUBLISHED 和 REPORT 通知。原 runner 业务 FAIL 不影响 CLI exit，现已由测试锁定为非零。
  - 结果：PASS；REG-01 Gate 关闭，单体 owner 表仍保留。下一步进入 HPA/依赖故障/恢复/性能实验，实验通过后再执行 DEL-01。
...
实验统一证据（2026-08-31；PERF-01 三接口最终复测 2026-09-01）：`docs/practice-2026/13-resilience-performance-experiments.md`；完整 `test:ci` 286/286，experiment scripts 5/5，交付包与性能组合定点测试 7/7。EXP-01 使用 Kubernetes v1.36.1 + 官方 metrics-server v0.9.0，Gateway 在 CPU 104% 时约 21 秒由 1→3，撤压/指标回落后约 30 秒经 3→2→1；脚本清理 HPA/metrics/load。EXP-02 对 live MySQL、SRS、MinIO 分别执行 failure/recovery，受影响域返回 503/500、其他服务 ready 200，恢复后业务 200。PERF-01 对推荐流、搜索、视频详情在同机/同脚本/同一逻辑数据下各跑单体与微服务 3 轮，共 4320 个计时请求、0 error；微服务相对单体：推荐 p95 +132.0% / RPS -50.0%，搜索 p95 -21.0% / RPS +39.1%，详情 p95 +146.3% / RPS -58.6%，结论为 1 项提升、2 项下降。统一 K8s 部署修复跨架构导入、MinIO revision 和 Secret restart 后从头 5/5 Ready；实验资源与 schema/user 精确清理。
...
| PERF-01 性能对比 | DONE | 推荐/搜索/详情三接口；同机、同一逻辑数据、预热、轮换顺序、3×2×3×240、并发16、完整响应体与签名一致 Gate | 4320/4320 200；推荐 p95 6.75→15.66ms，搜索 24.29→19.20ms，详情 9.55→23.52ms | PASS；0 error，1 项提升、2 项下降，全部 p95 < 1000ms；非生产容量承诺 | 本轮提交 |
...
| 2026-08-31 | EXP-01/02 + K8s 可重复部署 | 官方 metrics-server 离线构建、HPA 负载时间线；MySQL/SRS/MinIO 故障恢复；跨架构镜像导入、MinIO revision 与 Secret restart 修复 | HPA 1→3→2→1；3 类依赖 failure/recovery；K8s 5/5 Ready | PASS；关键值进入 `13-resilience-performance-experiments.md`；HPA/metrics/load、微服务 K8s 资源、PVC、四 schema/user 清理 |
| 2026-09-01 | PERF-01 三接口性能最终复测 | 将旧单路径 runner 扩展为推荐/搜索/详情矩阵；双目标统一 fixture，计时前校验 `id:title` 签名；轮换接口与目标顺序；在最新 `main@27f9425` 保存 18 行主证据 CSV并复制进交付包 | 三接口 × 单体/Gateway × 3 轮 × 240 = 4320 请求，0 error；推荐 p95 +132.0%、RPS -50.0%；搜索 p95 -21.0%、RPS +39.1%；详情 p95 +146.3%、RPS -58.6% | PASS；1 项提升、2 项下降；services browser 7/7、双目标 REG 12/12、三故障恢复与 rollback 同轮 PASS。先处理 buildx 缺失与 Docker VM ENOSPC，清理 0 引用中间镜像后最终 runner exit 0，容器/volume/端口清理完成 |
...
| 2026-09-01 | DEL-03-04 独立实体材料包 | 将 Docker/Compose、GitHub/Jenkins、K8s/Kustomize、Prisma/migration/seed/cutover、自动化测试、压力/韧性脚本和实验 CSV 以普通文件复制进 `delivery/03_devops`、`delivery/04_tests`；新增来源 Manifest、完整 checksum、生成器和防索引回退单测 | 03/04 来源副本 119/92；raw evidence 19；三份 checksum；Compose config 2/2；Kustomize render 2/2；delivery package 2/2；`npm run test:ci` 284/284；无软链接/目录外链接 | LOCAL PASS；实体文件基线 `main@481d683`，历史 raw evidence 保留 `run 33379394312 / head c909875`；本轮 API/E2E/镜像/真实 Kind `NOT RUN`，未连接共享数据库或集群；DEL-01 仍 `HUMAN EVIDENCE PENDING` |
| 2026-09-01 | DEL-03-04 PR #70 review blocker 修复 | 排除已生成 delivery 路径，保留 raw experiment 白名单；移除 Manifest 动态 commit/工作树字段；文本复制与哈希统一 LF，raw/binary 保留原始字节；quality job 增加 clean-checkout 生成后 diff 门禁 | 连续生成两次 hash 幂等；delivery package 2/2；03/04/raw checksum；Compose 2/2；Kustomize 2/2；干净 `npm ci` 后 `test:ci` 284/284；普通与 `core.autocrlf=true` clean checkout 均连续生成 2 次、diff 0、package 2/2 | LOCAL PASS；首次隔离 worktree 缺 `nest` 导致 build 127，`npm ci` 后原命令通过；review 两个 blocker 均已按原复现命令关闭，push 后请求 GloamingL 复审；API/E2E/镜像/真实 Kind 本轮仍 `NOT RUN` |
| 2026-09-01 | QA-CLOSE-01 全接口/浏览器/Actions 闭环 | 自动发现并逐条请求 126 个公开 Controller 路由；新增 services-mode UC01–UC06 串行浏览器回归且 Token 缺失直接失败；Actions 在 PR/main 均构建单体、四服务、Gateway 与 migration Git SHA 镜像并部署隔离 Kind；修复“全部”分类误传、关注状态跨服务丢失与 Runner 磁盘峰值 | `npm run test:ci` 284/284；隔离 MySQL `npm run test:api` 143/143（Controller 126/126、0 skip、0 5xx）；标准 Compose 最终 services suite 7 个场景、0 skip（本地扩展集曾 8/8 PASS）；本地 Kind 四 migration、单体 + 五服务/Gateway 全 Ready、0 restart、health/version PASS | REMOTE PASS；PR #73 run `33489462704` 4/4 jobs SUCCESS：API 143/143 与 Controller 126/126、services E2E 7/7、11 个 `caeb847` Git SHA 镜像、四 migration、单体/五服务/Gateway Ready、0 restart、三个 Artifact 和 cleanup 全部实检 |
| 2026-09-01 | DEL-01 技术总结报告 review 修复 | 将 `技术总结报告.md` 设为 `06_defense` 唯一权威汇总源，替代旧 `technical-summary.md` 并接入 README/PDF 生成器；修正不存在的 `PENDING_SUBMISSION`、区分整体 FFmpeg 与 services-mode content-media 的 FFprobe 验证能力；rebase 后同步最新三端点/4320 请求性能证据；正式/补充 QA 与 checksum 同步，并固定跨平台 checksum 文件名排序 | `scripts/verify-delivery-pdfs.py` 连续运行两次且目标路径 diff 0；delivery package 2/2；14 页 PDF 渲染、边界、空白页和 SHA-256 PASS；CRLF-aware diff check PASS | PASS；技术总结 PDF 为 14 页，正式/补充 QA 一致且生成幂等；DEL-01 仍为 `HUMAN EVIDENCE PENDING`；应用 lint/build/API/E2E 由最终 PR SHA 的远端 Actions 复跑 |
| 2026-09-02 | DOC-02 云原生/性能报告与课程 PDF | 新增云原生实验和三端点性能正式报告；保留课程指定 5 份 PDF，同时保留仓库既有 7-PDF 生成/QA 包；区分历史实验快照与当前集成基线，并明确 DEL-01 真人证据仍待补 | 5 PDF / 212 页 pdfplumber + Poppler 全页 QA；独立 QA/checksum；delivery package 2/2；verifier 双跑幂等；PR run `33535179397` 4/4；main run `33536573190` 4/4 | REMOTE PASS；PR #75 final head `876e1665`、squash merge `4912e6a9`；126 路由、public/services browser、11 个 SHA 镜像、Kind deploy/health、3 Artifacts、cleanup 全绿；DEL-01 仍为 `HUMAN EVIDENCE PENDING` |
| 2026-09-02 | TEST-01 前端测试交付同步 | 将新增的前端 Vitest 与 Vue 组件测试、jsdom 测试配置同步到 `delivery/04_tests` 实体材料包，并刷新来源清单与 checksum；修复 PR quality job 在 standalone delivery regeneration 阶段失败的问题；review 修复将 README 中误写为清单总数 `103` 的 `automation/` 文件数更正为 `95` | `npm run delivery:standalone`；`node --test test/unit/delivery-package.test.js` 2/2；`npm run lint:frontend`；Vitest 13 files/152 tests；`vue-tsc --noEmit`；`npm run test:ci` 414/414；PR run `33607275311` 4/4 | LOCAL PASS；交付包清单共 103 条，其中 `automation/` 95 条，生成器复跑后仅 README 对应 checksum 改变；完整 lint/build/requirements/unit/services/regression 414/414。PR head `5b21d90` 的 run `33607275311` REMOTE PASS：quality、143/143 API（126/126 公开路由）、public browser smoke、services-mode 7/7 和 Kind 部署均成功。首次 run `33605115771` 因副本未同步在 `test:ci` 前失败；本次文档修复 push 后仍需最终 head CI 复跑和非作者 review |
