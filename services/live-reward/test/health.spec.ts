...
describe('live-reward runtime', () => {
  it('reports unavailable when production persistence is not configured', async () => {
    const previousDatabaseUrl = process.env.LIVE_REWARD_DATABASE_URL;
    delete process.env.LIVE_REWARD_DATABASE_URL;
    try {
      ...
      if (previousDatabaseUrl === undefined) delete process.env.LIVE_REWARD_DATABASE_URL;
      else process.env.LIVE_REWARD_DATABASE_URL = previousDatabaseUrl;
    }
  });
