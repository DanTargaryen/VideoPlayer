...
describe('live-reward HTTP adapters and internal auth', () => {
  it('exposes the complete room/session HTTP lifecycle contract', async () => {
    ...
    expect((await request('/api/v1/lives/center/overview')).status).toBe(200);
    expect((await request('/api/v1/lives/categories')).status).toBe(200);
    ...
    expect((await request('/api/v1/lives/plaza?category=tech')).status).toBe(200);
    expect((await request('/api/v1/lives/hot?limit=4')).status).toBe(200);
    expect((await request(`/api/v1/lives/rooms/${roomId}/frame`, { method: 'POST', body: JSON.stringify({ image: 'data:image/jpeg;base64,AA==' }) })).status).toBe(200);
    expect((await (await request(`/api/v1/lives/rooms/${roomId}/frame`)).json()).data.image).toBe('data:image/jpeg;base64,AA==');
    const viewer = await request(`/api/v1/lives/rooms/${roomId}/viewers`, { method: 'POST', body: JSON.stringify({ viewerId: 'http-viewer' }) });
    expect(viewer.status).toBe(200);
    const viewerPayload = (await viewer.json()).data as { sessionId: number; viewerId: string };
    const sessionId = viewerPayload.sessionId;
    expect((await request(`/api/v1/lives/rooms/${roomId}/viewers/${viewerPayload.viewerId}/offer`, { method: 'POST', body: JSON.stringify({ type: 'offer', sdp: 'viewer-offer' }) })).status).toBe(200);
    const pending = await (await request(`/api/v1/lives/rooms/${roomId}/publisher/pending-viewers`)).json();
    expect(pending.data).toHaveLength(1);
    expect((await request(`/api/v1/lives/rooms/${roomId}/viewers/${viewerPayload.viewerId}/answer`, { method: 'POST', body: JSON.stringify({ type: 'answer', sdp: 'publisher-answer' }) })).status).toBe(200);
    expect((await (await request(`/api/v1/lives/rooms/${roomId}/viewers/${viewerPayload.viewerId}/answer`)).json()).data.ready).toBe(true);
    expect((await request(`/api/v1/lives/sessions/${sessionId}`)).status).toBe(200);
    const message = await request(`/api/v1/lives/rooms/${roomId}/messages`, { method: 'POST', body: JSON.stringify({ content: 'hello' }) });
    expect(message.status).toBe(200);
    expect((await message.json()).data).toMatchObject({ roomId, content: 'hello', sender: { id: 7, nickname: 'anchor' } });
