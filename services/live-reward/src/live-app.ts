...
function userFromHeaders(request: IncomingMessage): User | null { const id = Number(request.headers['x-user-id']); if (!Number.isInteger(id) || id < 1) return null; const nickname = typeof request.headers['x-user-nickname'] === 'string' ? request.headers['x-user-nickname'] : `user-${id}`; return { id, nickname }; }
