...
async function dispatch(app: LiveApplication, request: IncomingMessage, url: URL, body: Record<string, unknown>, requestId: string): Promise<unknown> {
  ...
  if (method === 'POST' && path === '/api/v1/gift-coins/video') return app.coinVideo(requireUser(user).id, stringValue(body.videoId), numberValue(body.amount), requestId);
  const giftVideoMatch = path.match(/^\/api\/v1\/gift-coins\/video\/([^/]+)$/); if (giftVideoMatch && method === 'POST') return app.coinVideo(requireUser(user).id, decodeURIComponent(giftVideoMatch[1]), numberValue(body.amount), requestId);
  const coinMatch = path.match(/^\/api\/v1\/videos\/([^/]+)\/coin$/); if (coinMatch && method === 'POST') return app.coinVideo(requireUser(user).id, decodeURIComponent(coinMatch[1]), numberValue(body.amount), requestId);
  const internalCoinMatch = path.match(/^\/internal\/v1\/(?:videos\/([^/]+)\/coin|coins\/videos\/([^/]+))$/); if (internalCoinMatch && method === 'POST') { authorizeInternal(request, 'live.ledger.write'); return app.coinVideo(integerValue(body.userId), decodeURIComponent(internalCoinMatch[1] ?? internalCoinMatch[2]), numberValue(body.amount), requestId); }
  const internalWalletMatch = path.match(/^\/internal\/v1\/users\/(\d+)\/wallet$/); if (internalWalletMatch && method === 'GET') { authorizeInternal(request, 'live.wallet.read'); return app.wallet(Number(internalWalletMatch[1])); }
