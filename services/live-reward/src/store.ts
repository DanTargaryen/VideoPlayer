...
export class IdempotencyConflictError extends Error {
  constructor(message = 'requestId conflicts with an existing payload') {
    super(message);
...
export interface Store {
  ...
  getReplayByRequestId(requestId: string): Promise<ReplayRecord | null>;
...
export class MemoryStore implements Store {
  ...
  async createReplay(input: Omit<ReplayRecord, 'id' | 'createdAt' | 'updatedAt' | 'attempts' | 'lastError' | 'nextRetryAt'>): Promise<ReplayRecord> {
    const existing = await this.getReplayBySession(input.sessionId)
      ?? await this.getReplayByKey(input.objectKey)
      ?? await this.getReplayByRequestId(input.requestId);
    if (existing) return assertReplayPayload(existing, input);
    const now = new Date();
  ...
  async getReplayByRequestId(requestId: string): Promise<ReplayRecord | null> { return Array.from(this.replays.values()).find((item) => item.requestId === requestId) ?? null; }
  ...
  async claimDaily(userId: number, claimDate: Date, requestId: string): Promise<{ claimed: boolean; amount: number; balance: number }> {
    const requestPayload = dailyRequestPayload(userId, claimDate);
    ...
    if (existingTransaction) {
      assertTransactionPayload(existingTransaction, requestPayload);
      return { claimed: false, amount: 0, balance: existingTransaction.balanceAfter };
    }
    ...
    return this.credit(userId, 2, 'DAILY_CLAIM', null, requestId, requestPayload);
  }
  ...
  async claimMilestone(userId: number, milestone: number, requestId: string): Promise<{ claimed: boolean; amount: number; balance: number; message?: string }> {
    const requestPayload = milestoneRequestPayload(userId, milestone);
    const existingTransaction = this.transactions.find((item) => item.requestId === requestId);
    if (existingTransaction) {
      assertTransactionPayload(existingTransaction, requestPayload);
      return { claimed: false, amount: 0, balance: existingTransaction.balanceAfter };
  ...
  async coinVideo(userId: number, videoId: number, amount: number, requestId: string): Promise<{ amount: number; balance: number; userVideoCoinCount: number }> {
    ...
    if (existingTransaction) {
      assertTransactionPayload(existingTransaction, requestPayload);
      return { amount: Math.abs(existingTransaction.amount), balance: existingTransaction.balanceAfter, userVideoCoinCount: this.contributions.get(`${videoId}:${userId}`) ?? 0 };
    }
    const key = `${videoId}:${userId}`;
    const current = this.contributions.get(key) ?? 0;
    if (current + amount > 2) throw new Error('每个视频最多投币 2 个');
    const balance = await this.debit(userId, amount, 'VIDEO_COIN', videoId, requestId, requestPayload);
  ...
  async gift(userId: number, amount: number, requestId: string): Promise<{ amount: number; balance: number }> {
    const requestPayload = giftRequestPayload(userId, amount);
    const existing = this.transactions.find((item) => item.requestId === requestId);
    if (existing) {
      assertTransactionPayload(existing, requestPayload);
      return { amount: Math.abs(existing.amount), balance: existing.balanceAfter };
    }
  ...
  private async debit(userId: number, amount: number, type: TransactionType, videoId: number | null, requestId: string, requestPayload: string) {
    ...
    this.transactions.push({ id: this.nextTransactionId++, userId, type, amount: -amount, balanceAfter: next, videoId, requestId, requestPayload, createdAt: new Date() });
    return next;
  }
...
export class PrismaStore implements Store {
  ...
  async createReplay(input: Omit<ReplayRecord, 'id' | 'createdAt' | 'updatedAt' | 'attempts' | 'lastError' | 'nextRetryAt'>): Promise<ReplayRecord> {
    ...
    try {
      ...
      if (isUniqueError(error)) {
        ...
        if (byKey) return assertReplayPayload(byKey, input);
        const byRequestId = await this.getReplayByRequestId(input.requestId);
        if (byRequestId) return assertReplayPayload(byRequestId, input);
      }
      throw error;
    }
  }
  ...
  async getReplayByRequestId(requestId: string): Promise<ReplayRecord | null> { return this.prisma.replayRegistration.findUnique({ where: { requestId } }); }
  ...
  async claimDaily(userId: number, claimDate: Date, requestId: string) {
    ...
    if (existing) {
      assertTransactionPayload(existing, requestPayload);
      return { claimed: false, amount: 0, balance: existing.balanceAfter };
    }
    try {
      return await this.prisma.$transaction(async (tx) => {
        ...
        await tx.coinTransaction.create({ data: { userId, type: 'DAILY_CLAIM', amount: 2, balanceAfter: account.balance, requestId, requestPayload } });
        return { claimed: true, amount: 2, balance: account.balance };
      });
    } catch (error) {
      if (isUniqueError(error)) {
        const replay = await this.prisma.coinTransaction.findUnique({ where: { requestId } });
        if (replay) {
          assertTransactionPayload(replay, requestPayload);
          return { claimed: false, amount: 0, balance: replay.balanceAfter };
        }
        return { claimed: false, amount: 0, balance: await this.getBalance(userId) };
      }
      throw error;
    }
  }
  ...
  async claimMilestone(userId: number, milestone: number, requestId: string) {
    ...
    try {
      return await this.prisma.$transaction(async (tx) => {
        ...
        await tx.coinTransaction.create({ data: { userId, type: 'STREAK_REWARD', amount: 10, balanceAfter: account.balance, requestId, requestPayload } });
      ...
      if (isUniqueError(error)) {
        ...
        return { claimed: false, amount: 0, balance: await this.getBalance(userId), message: '该里程碑已领取' };
      }
      throw error;
    }
  }
  async coinVideo(userId: number, videoId: number, amount: number, requestId: string) {
    ...
    if (existingTx) {
      assertTransactionPayload(existingTx, requestPayload);
      return { amount: Math.abs(existingTx.amount), balance: existingTx.balanceAfter, userVideoCoinCount: (await this.prisma.videoCoinContribution.findUnique({ where: { videoId_userId: { videoId, userId } } }))?.amount ?? 0 };
    }
    try {
      return await this.prisma.$transaction(async (tx) => {
        ...
        await tx.coinTransaction.create({ data: { userId, type: 'VIDEO_COIN', amount: -amount, balanceAfter: account.balance, videoId, requestId, requestPayload } });
      ...
      if (isUniqueError(error)) {
        ...
        if (existing) {
          assertTransactionPayload(existing, requestPayload);
          return { amount: Math.abs(existing.amount), balance: existing.balanceAfter, userVideoCoinCount: (await this.prisma.videoCoinContribution.findUnique({ where: { videoId_userId: { videoId, userId } } }))?.amount ?? 0 };
        }
      }
      throw error;
    }
  ...
  async gift(userId: number, amount: number, requestId: string) {
    ...
    try {
      return await this.prisma.$transaction(async (tx) => {
        const account = await tx.coinAccount.upsert({ where: { userId }, create: { userId, balance: 10 - amount }, update: { balance: { decrement: amount } } });
        if (account.balance < 0) throw new Error('余额不足，请每日打卡获取货币');
        await tx.coinTransaction.create({ data: { userId, type: 'LIVE_GIFT', amount: -amount, balanceAfter: account.balance, requestId, requestPayload } });
...
function assertReplayPayload(
  ...
  input: Pick<ReplayRecord, 'sessionId' | 'objectKey' | 'requestId' | 'mimeType'>,
): ReplayRecord {
  if (
    ...
    throw new IdempotencyConflictError('Replay request conflicts with an existing session, objectKey, or requestId payload');
  }
  return existing;
}

function assertTransactionPayload(existing: Pick<TransactionRecord, 'requestPayload'>, requestPayload: string): void {
  if (existing.requestPayload !== requestPayload) {
    throw new IdempotencyConflictError('Ledger requestId conflicts with an existing user, type, resource, or amount');
  }
}

function dailyRequestPayload(userId: number, claimDate: Date): string {
  return `daily:${userId}:${claimDate.toISOString().slice(0, 10)}`;
}

function milestoneRequestPayload(userId: number, milestone: number): string {
...
function giftRequestPayload(userId: number, amount: number): string {
  return `gift:${userId}:${amount}`;
}
