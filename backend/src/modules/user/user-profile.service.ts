...
export class UserProfileService {
  ...
  async buildAndSaveProfile(userId: number): Promise<UserRecommendationProfileDto> {
    ...
    for (const item of likes) {
      this.addScore(categoryScores, this.resolveVideoCategoryId(item.video.category), USER_PROFILE_BEHAVIOR_WEIGHTS.category.like);
      this.addScore(creatorScores, item.video.creatorId, USER_PROFILE_BEHAVIOR_WEIGHTS.creator.like);
    }

    for (const item of favorites) {
      this.addScore(categoryScores, this.resolveVideoCategoryId(item.video.category), USER_PROFILE_BEHAVIOR_WEIGHTS.category.favorite);
      this.addScore(creatorScores, item.video.creatorId, USER_PROFILE_BEHAVIOR_WEIGHTS.creator.favorite);
    }

    for (const item of comments) {
      this.addScore(categoryScores, this.resolveVideoCategoryId(item.video.category), USER_PROFILE_BEHAVIOR_WEIGHTS.category.comment);
      this.addScore(creatorScores, item.video.creatorId, USER_PROFILE_BEHAVIOR_WEIGHTS.creator.comment);
    }

    for (const item of danmakus) {
      this.addScore(categoryScores, this.resolveVideoCategoryId(item.video.category), USER_PROFILE_BEHAVIOR_WEIGHTS.category.danmaku);
    ...
    for (const item of createdVideos) {
      this.addScore(categoryScores, this.resolveVideoCategoryId(item.category), USER_PROFILE_BEHAVIOR_WEIGHTS.category.createdVideo);
    }
  ...
  private addScore(scoreMap: Map<number, number>, key: number | undefined, score: number) {
    if (!key) {
      return;
    }
  ...
  private resolveVideoCategoryId(categoryCode?: string | null) {
    return resolveCategoryId(categoryCode ?? undefined);
  }
