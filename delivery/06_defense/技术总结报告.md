...
- 主流程：创作者选择视频和封面，填写标题、简介和分区；content-media 接收 multipart 文件并校验类型、大小、扩展名；原始对象写入 MinIO，VideoAsset 登记 objectKey 和媒体元数据；FFprobe 解析时长，FFmpeg 负责转码或抽帧；稿件保存为草稿并提交审核；治理服务创建审核任务；管理员审核通过后，content-media 将视频置为 PUBLISHED，视频进入推荐、搜索和播放范围。
- 异常流程：上传超时、非法媒体、MinIO 不可用、FFmpeg/FFprobe 失败、重复提交、治理服务不可用、管理员驳回、创作者修改后重新提交。
...
- 最终汇报重点：该用例同时展示对象存储、FFmpeg 媒体处理、稿件状态机、跨角色审核、跨服务调用和失败恢复。
...
| --- | --- | --- | --- |
| 提交审核 | content -> governance | 2 秒；网络错误最多重试 2 次；requestId 幂等 | 稿件保持 DRAFT/PENDING_SUBMISSION，不直接发布 |
...
- 请求：`GET /api/v1/feeds/recommend?page=1&pageSize=1`；
- 两目标预检均返回一条已发布记录；
- 每目标预热 20 次；
- 三轮交叉执行，避免固定先后顺序偏差；
- 每目标每轮 240 请求；
...
| 目标 | 轮次 | RPS | mean | p50 | p95 | p99 | max | 错误 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| monolith | 1 | 1805.41 | 8.71 ms | 7.37 ms | 14.85 ms | 56.00 ms | 56.14 ms | 0 |
| microservice-gateway | 1 | 1108.22 | 14.21 ms | 13.23 ms | 22.32 ms | 31.40 ms | 43.50 ms | 0 |
| microservice-gateway | 2 | 1435.25 | 11.01 ms | 10.84 ms | 15.57 ms | 17.39 ms | 17.94 ms | 0 |
| monolith | 2 | 2334.84 | 6.75 ms | 6.57 ms | 9.44 ms | 9.83 ms | 10.02 ms | 0 |
| monolith | 3 | 2455.14 | 6.37 ms | 6.55 ms | 7.44 ms | 7.91 ms | 8.41 ms | 0 |
| microservice-gateway | 3 | 1496.81 | 10.48 ms | 9.77 ms | 15.24 ms | 18.31 ms | 19.24 ms | 0 |
