...
性能实验在同机、同逻辑数据、同脚本、并发 16 下，对推荐流、搜索、视频详情分别进行单体/Gateway 各三轮对比，共 4320 次正式请求、0 error。推荐流 p95 `6.75→15.66 ms`、RPS `3009.32→1505.41`，性能下降；搜索 p95 `24.29→19.20 ms`、RPS `935.16→1301.26`，性能提升；详情 p95 `9.55→23.52 ms`、RPS `2320.96→961.41`，性能下降。结果是 2 个接口下降、1 个提升，说明微服务拆分的影响与路径特征有关，不是统一的性能提升。
...
A–E 默认成员映射已按用户提供的成员/学号表记录为林明、刘钟屹、李晓萌、张壮志、王一涵。仍未关闭的是必须由真人提供的课程管理证据：教师确认原件、ARCH 实际参会/会议原件与个人备份人、非作者 clean-machine 复现、五人实际贡献核对/权重和签字、5–8 分钟备用录屏及全员计时演练。在这些原件到位前，不把 DEL-01 标记为完成。
